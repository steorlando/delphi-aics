...
type Phase2VoteNotesDeleteBuilder = {
  delete(): {
    eq(column: string, value: string): {
      eq(column: string, value: string): {
        eq(column: string, value: string): {
...
export async function deleteExpertPhase2VoteNoteAction(
  _previousState: SaveExpertPhase2VoteNoteFormState,
  formData: FormData,
): Promise<SaveExpertPhase2VoteNoteFormState> {
  const { profile } = await getAuthContext();
  ...
  if (!profile || !profile.is_active || profile.role !== "expert") {
    return {
      status: "error",
      message: "Solo gli esperti autenticati possono eliminare i commenti ai voti.",
    };
  }
  ...
  if (!commentId) {
    return {
      status: "error",
      message: "Commento da modificare non valido.",
    };
  }
  ...
  const voteNotesTable = supabase.from(
    "expert_section_comment_vote_notes",
  ) as unknown as Phase2VoteNotesDeleteBuilder;
  const deleteQuery = voteNotesTable
    .delete()
    .eq("consultation_id", consultationId)
    .eq("comment_id", commentId)
  ...
  if (!data) {
    return {
      status: "error",
      message: "Nessun commento da eliminare trovato per questo voto.",
    };
  }
